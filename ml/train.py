"""
OpenGuitarChordProject - 训练脚本 (NVIDIA GPU版，GPU梅尔频谱)
使用方法:
...
def train_one_epoch(model, loader, criterion, optimizer, device,
                    mel_transform, db_transform, verbose=False):
    model.train()
    total_loss, total_acc, n = 0, 0, 0
    
    for batch_idx, (waveform, label) in enumerate(loader):
        t0 = time.time()
        ...
        with torch.no_grad():
            ...
            # 标准化
            mel = (mel - mel.mean(dim=(2,3), keepdim=True)) / \
                  (mel.std(dim=(2,3), keepdim=True) + 1e-8)
            t4 = time.time()
        ...
        if verbose and batch_idx < 3:
            print(f"[Batch {batch_idx}] 数据送GPU: {(t1-t0)*1000:.2f}ms | 设备: {waveform.device}")
            print(f"[Batch {batch_idx}] 梅尔频谱计算: {(t2-t1)*1000:.2f}ms | 设备: {mel.device}")
            print(f"[Batch {batch_idx}] dB转换: {(t3-t2)*1000:.2f}ms")
            print(f"[Batch {batch_idx}] 对齐/标准化: {(t4-t3)*1000:.2f}ms")
            print(f"[Batch {batch_idx}] 模型训练: {(t5-t4)*1000:.2f}ms | 输出设备: {out.device}")
            print(f"[Batch {batch_idx}] ===== 总时间: {(t5-t0)*1000:.2f}ms =====\n")
            if batch_idx == 2:
                print("... (后续batch省略打印) ...\n")
...
@torch.no_grad()
def evaluate(model, loader, criterion, device, mel_transform, db_transform):
    model.eval()
    total_loss, total_acc, n = 0, 0, 0
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_acc = train_one_epoch(
            ...
            mel_transform, db_transform, verbose=args.verbose
        )
        val_loss, val_acc, val_preds, val_labels = evaluate(
            model, val_loader, criterion, device,
            mel_transform, db_transform
        )
        scheduler.step()
